...
if [ -z ${1+x} ]; then
  ...
  ./src/get_disks.py `grep disk $vmfile | sed 's/disk = \[//g' | sed 's/file://g' | sed 's/xv\w\+//g' | sed 's/,,w!//g' | sed 's/,,w//g' | sed 's/ //g' | sed 's/.$//' | sed 's/\'//g'`
fi
...
# sed 's/\'//g'         : Remove all apostrophes in the string

#disks_str=`grep disk $vmfile | sed 's/disk = \[//g' | sed 's/file://g' | sed 's/xv\w\+//g' | sed 's/,,w!//g' | sed 's/,,w//g' | sed 's/ //g' | sed 's/.$//' | sed 's/\'//g'`
